...
export default function Home() {
  ...
  const [maxNextMatch, setNextMatch] = useState(4);
    ...
    <>
      ...
      <div className="bg-black w-full min-h-screen h-full px-6">
        <div className="text-center p-8 space-y-4">
          ...
          <p className="text-sm md:text-xl">Update world cup QATAR 2022 secara Realtime dari Genggaman Anda.</p>
        </div>
        <div className="flex justify-center w-full">
          <div className="max-w-[550px] w-full border border-dashed rounded-lg p-6">
            <h2 className="text-center text-2xl pb-4">Current Match</h2>
        ...
        <div className="py-8">
          <div className="max-w-[550px] mx-auto w-full">
            <h2 className="text-2xl md:text-3xl">Next Match</h2>
            <div className="py-4">
              <ul className="space-y-4">
                {nextMatch.slice(0, 4).map((item, index) => <li key={item.id} className="flex gap-4">
                  <div className="w-4">
                    <span className="relative inline-flex rounded-full h-3 w-3 bg-lime-500"></span>
                    {index !== maxNextMatch - 1 && <div className="h-full px-1 pb-3 pt-1">
                      <span className="block w-[4px] opacity-50 bg-amber-50 h-full"></span>
                    </div>}
                  </div>
                  <div>
                    <p className="text-lime-400 text-sm">{dayjs(item.datetime).format("MMMM DD")}</p>
                    <div className="py-2">
        ...
        <div className="py-8">
          <div className="max-w-[550px] mx-auto w-full">
            <h2 className="text-2xl md:text-3xl">Completed Match</h2>
            <div className="py-4">
              <ul className="space-y-4">
              ...
              {maxCompletedMatch == 4 ? <div className="w-full flex justify-center p-2">
                <button onClick={() => setMaxCompletedMatch(completedMatch.length)} className="text-sm px-6 py-2 border border-dashed rounded-full hover:bg-lime-600 hover:shadow-lg hover:opacity-80 transition duration-300 ease-in-out">Show more</button>
              </div> : <div className="w-full flex justify-center p-2">
                <button onClick={() => setMaxCompletedMatch(4)} className="text-sm px-6 py-2 border border-dashed rounded-full hover:bg-lime-600 hover:shadow-lg hover:opacity-80 transition duration-300 ease-in-out">Show less</button>
              </div>}
